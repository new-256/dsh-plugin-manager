...
import { existsSync, readFileSync, writeFileSync, copyFileSync, unlinkSync, rmSync, mkdirSync, lstatSync, renameSync } from 'node:fs';
 ...
 */
function safeWritePatchFile(newContent) {
...
function patchToggleEntry(entryId, disabled) {
  ...
  const originalText = readFileSync(PATCH_PATH, 'utf8');
...
function patchAddInstallEntry(pkgName, entryId) {
  const originalText = existsSync(PATCH_PATH) ? readFileSync(PATCH_PATH, 'utf8') : '';
  const addition = [
    '',
  ...
  const newText = originalText.trimEnd() + '\n' + addition + '\n';
  safeWritePatchFile(newText);
}
    ...
    const handler = async (req, res) => {
      ...
      try {
        ...
        if (pathname === '/plugin-manager/api/version' && req.method === 'GET') {
          return jsonRes(200, { ok: true, version: PLUGIN_VERSION });
        }
